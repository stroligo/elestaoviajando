...
export function EditTrip({ setActiveComponent, selectedTripId }) {
      ...
      try {
        ...
        setFormData({
          ...
          images: trip.images.map((image) =>
            image.startsWith('http') ? image : `${CLOUDINARY_BASE_URL}${image}`,
          ),
        });
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetchWithAuth(
        `https://elestaoviajando.onrender.com/api/trips/${selectedTripId}`,
        {
          method: 'PUT',
          body: JSON.stringify({
            ...formData,
